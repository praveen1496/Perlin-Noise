...
class MapGen_Main(MapGen_Base):  # get base perlin height, make terrain and water
    ...
    def Generate(self, x, y):
        ...
        basePerlinValue = (snoise2(float(x) * perlinScale, float(y) * perlinScale, octaves=8, persistence=0.5,
                                   lacunarity=2.0, repeatx=2048, repeaty=2048, base=perlinOffset) + 1) / 2.0;
        ...
        if (heightMap[x][y] > landThreshold):  # land
            ...
            detailPerlinValue = (snoise2(float(x) * perlinScale, float(y) * perlinScale, octaves=12, persistence=0.8,
                                         lacunarity=2.0, repeatx=2048, repeaty=2048, base=perlinOffset) + 1) / 2.0;

            normalizedHeight = (detailPerlinValue - landThreshold);
            normalizedHeight *= normalizedHeight * normalizedHeight;  # normalized height ^3

            noiseValue = (snoise2(float(x) * colorPerlinScale, float(y) * colorPerlinScale, octaves=2, persistence=0.5,
                                  lacunarity=2.0, repeatx=2048, repeaty=2048, base=perlinOffset) + 1) / 2.0;
            ...
            noiseValue = (snoise2(float(x) * waterNoisePerlinScale, float(y) * waterNoisePerlinScale, octaves=2,
                                  persistence=0.5, lacunarity=2.0, repeatx=2048, repeaty=2048,
                                  base=perlinOffset) + 1) / 2.0;
